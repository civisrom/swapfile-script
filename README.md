...
Автоматическая настройка гибридной подкачки (swapfile + zram) для Debian/Ubuntu VPS с малым объёмом RAM (512 МБ – 4 ГБ).
...
╚══════════════════════════════════════════════════════╝
  ...
  ├──────┼──────────────┼───────────┼───────┼──────────┼────────────┤
  │ 0.5  │ 1024 MB      │   100%    │ zstd  │   100    │    100     │
  │  1   │  512 MB      │   100%    │ zstd  │   100    │    100     │ << recommended
  │  2   │ 1024 MB      │    75%    │ zstd  │   100    │    100     │
  │  3   │ 1024 MB      │    60%    │ zstd  │   100    │     80     │
  │  4   │ 1536 MB      │    50%    │ zstd  │   100    │     80     │
  ├──────┼──────────────┼───────────┼───────┼──────────┼────────────┤
  │  6   │ Manual input — set each parameter yourself                │
  └──────┴──────────────┴───────────┴───────┴──────────┴────────────┘

  Select template (0.5, 1-4, or 6 for manual):
```

### Выбор шаблона (0.5, 1-4)
...
При выборе `6` скрипт последовательно запросит каждый параметр с подсказками:
...
| Параметр | 512 МБ RAM | 1 ГБ RAM | 2 ГБ RAM | 3 ГБ RAM | 4 ГБ RAM |
|---|---|---|---|---|---|
| Swap-файл | 1024 МБ | 512 МБ | 1024 МБ | 1024 МБ | 1536 МБ |
| zram PERCENT | 100% | 100% | 75% | 60% | 50% |
| ALGO | zstd | zstd | zstd | zstd | zstd |
| PRIORITY | 100 | 100 | 100 | 100 | 100 |
| swappiness | 100 | 100 | 100 | 80 | 80 |
...
Options:
  --ram N             Шаблон для N ГБ RAM (0.5, 1, 2, 3, 4)
